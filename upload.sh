...
help() {
    echo
    echo "upload.sh [destination IP] [Path to .fw file] [Path to ssh key]"
    echo
    ...
    echo "Default ssh key destination is './.ssh/heartbeat_ssh'"
    exit 1
}
...
[ -n "$SSH_KEY" ] || SSH_KEY=$(ls ./.ssh/heartbeat_ssh 2> /dev/null | head -n 1)
[ -n "$SSH_KEY" ] || (echo "Error: error finding ssh key."; help)
...
SSH_OPTIONS="-o PreferredAuthentications=publickey -i $SSH_KEY"
